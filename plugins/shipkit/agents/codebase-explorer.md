...
- **Cap at 20 files per task** — if more files need reading, report back and ask (deep
  end-to-end traces belong to the `tracer` agent, which `/shipkit:walkthrough` uses)
